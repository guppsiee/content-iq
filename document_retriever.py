...
class DocumentRetriever:
    ...
    def semantic_search(self, query: str, container_name: str, top_k: int = 3) -> List[Dict]:
        ...
        # Pair each chunk with its similarity and index
        chunk_data = [
            (idx, similarities[idx], metadata[idx])
            for idx in range(len(metadata))
        ]

        # Group by (container, blob_name) and keep only the chunk with the highest similarity for each file
        best_chunk_per_file = {}
        for idx, sim, meta in chunk_data:
            doc_key = (meta['container'], meta['blob_name'])
            if doc_key not in best_chunk_per_file or sim > best_chunk_per_file[doc_key][0]:
                best_chunk_per_file[doc_key] = (sim, idx)

        # Sort the best chunks by similarity and take the top_k
        top_chunks = sorted(best_chunk_per_file.values(), key=lambda x: x[0], reverse=True)[:top_k]
